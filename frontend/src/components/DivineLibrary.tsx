...
import { X, Search, Book, Users, ChevronRight, ExternalLink, MapPin, Languages, BookOpen } from 'lucide-react';
...
export default function DivineLibrary({ onClose }: DivineLibraryProps) {
    const [activeTab, setActiveTab] = useState<'names' | 'places' | 'wordstudy' | 'kidsstories'>('names');
                            ...
                            <div className="w-12 h-12 rounded-xl bg-gold-500/20 flex items-center justify-center">
                                {activeTab === 'names' ? <Users className="w-6 h-6 text-gold-400" /> :
                                    activeTab === 'places' ? <MapPin className="w-6 h-6 text-gold-400" /> :
                                        activeTab === 'wordstudy' ? <Languages className="w-6 h-6 text-gold-400" /> :
                                            <BookOpen className="w-6 h-6 text-gold-400" />}
                            </div>
                            <div>
                                <h2 className="text-2xl font-bold text-gold-200">Small References</h2>
